...
name = input("Enter nickname: ") # вводим имя перед началом игры
...
w, h, fps, level, step = 800, 800, 4, 1, 40 # разделяем окно на 400 квадратиков, 20 на 20
screen = pg.display.set_mode((w, h))
pg.display.set_caption('Snake Game')
bg = pg.image.load("C:\\PP2\\Lab10\\image\\background.jpg")
bg = pg.transform.scale(bg, (w, h))
gameover = pg.image.load("C:\\PP2\\Lab10\\image\\game_over.jpg")
gameover = pg.transform.scale(gameover, (390, 390))
is_running, lose, paused = True, False, False
clock = pg.time.Clock()
score = pg.font.SysFont("Verdana", 20)
surf = pg.Surface((380, 390))
cur_sc = 0
timmer = 5000
rush = False
lose = False
...
class Food:
    ...
    def draw2(self):
        ...
        self.y = randrange(0, h, step)
        self.r = randint(1, 3)
        self.image = pg.image.load(f'C:\\PP2\\Lab10\\image\\food{self.r}.png')
...
class Snake:
    ...
    def collide_food(self, f:Food):
        ...
        if self.body[0][0] == f.x and self.body[0][1] == f.y: # если координаты головы змейки совпадают с координатами еды
            self.score += f.r # добавляем разные очки соответствующий разным видам еды
            total += f.r
            global rush
            rush = True
...
class Wall:
    def __init__(self, x, y):
        self.x, self.y = x, y
        self.pic = pg.image.load("C:\\PP2\\Lab10\\image\\wall.png")
...
s = Snake()
f = Food(pg.image.load(f'C:\\PP2\\lab10\\image\\food{randint(1,3)}.png'))
disappear(5000)
...
while is_running:
    ...
    my_walls = open(f'C:\PP2\Lab10\wall{level}.txt', 'r').readlines() # читает каждую линию как отдельный лист
    walls = []
    ...
    f.draw()
    s.draw()
    s.move(events) # нажать клавишу чтобы начать игру
    s.collide_food(f)
    s.self_collide()
    s.check_food(f)
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN:
                ...
                if event.key == pg.K_c:
                    ...
                    '''
                    current.execute(sql, [total, level, name])
                    config.commit()  
    ...
    while lose:
        ...
        screen.blit(surf, (200, 200))
        cntr = score.render(f'Your score is {total}', True, 'white')
...
sql = '''
    UPDATE users SET score = %s, level = %s WHERE username = %s;
'''
current.execute(sql, [total, l, name])
config.commit()
current.close()
config.close()